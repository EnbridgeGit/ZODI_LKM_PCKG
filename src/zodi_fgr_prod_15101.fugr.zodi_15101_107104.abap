...
*"     VALUE(IV_P_COMPANY_CODE) TYPE  STRING OPTIONAL
*"  TABLES
...
*Cursor Declaration
*********************
DATA: lv_FAGLFLEXT_dbcur  TYPE cursor.
************************
*Hash value comparison
************************
lv_delimiter = iv_delimiter.
IF  iv_hashvalue NE '1256326052'.
CLEAR et_file_return.
...
************************************************
OPEN CURSOR WITH HOLD lv_FAGLFLEXT_dbcur FOR
SELECT RBUKRS
RCLNT
RBUSA
RFAREA
...
TIMESTAMP
FROM  FAGLFLEXT
WHERE RLDNR =  IV_P_LEDGER_CODE
 AND RRCTY = IV_P_RECORD_TYPE
 AND RYEAR = IV_P_YEAR.
 "AND RBUKRS = IV_P_COMPANY_CODE.
DO.
************************************************
...
FETCH NEXT CURSOR lv_FAGLFLEXT_dbcur
APPENDING TABLE tt_FAGLFLEXT
************************************************************
...
*********************************************
*Read data from child table T001
*********************************************
**************************************************
...
INTO wa_FAGLFLEXT.
MOVE wa_FAGLFLEXT-D1_RBUKRS
 TO wa_final-D1_RBUKRS.
MOVE wa_FAGLFLEXT-D3_RCLNT
 TO wa_final-D3_RCLNT.
...
ENDLOOP.
free : tt_FAGLFLEXT.
sort tt_final_tmp1 by D1_RBUKRS.
sort tt_T001 by D2_BUKRS.
***********************************
...
IF wa_T001-D2_BUKRS =
 wa_final-D1_RBUKRS.
MOVE wa_T001-D2_BUKRS
 TO wa_final-D2_BUKRS.
MOVE wa_T001-D5_LAND1
 TO wa_final-D5_LAND1.
MOVE wa_T001-D7_WAERS
 TO wa_final-D7_WAERS.
...
ENDIF.
ENDLOOP.
free : tt_final_tmp1, tt_T001.
************************************************
*Join Order 2 : LEFT OUTER JOIN
************************************************
sort tt_final1 by D3_RCLNT.
sort tt_T000 by D4_MANDT.
LOOP AT tt_final1
INTO wa_final.
lv_FLAG = ''.
read table tt_T000
...
IF wa_T000-D4_MANDT =
 wa_final-D3_RCLNT.
MOVE wa_T000-D4_MANDT
 TO wa_final-D4_MANDT.
...
*CLEAR wa_final.
ENDLOOP.
free : tt_final1, tt_T000.

************************************************
****Join Order 3 : INNER JOIN
************************************************
sort tt_final2 by D5_LAND1.
sort tt_T005 by D6_LAND1.
CLEAR wa_final.
LOOP AT tt_final2
INTO wa_final.
CLEAR : wa_T005.
...
IF wa_T005-D6_LAND1
 = wa_final-D5_LAND1.
MOVE wa_T005-D6_LAND1
 TO wa_final-D6_LAND1.
...
ENDIF.
ENDLOOP.
free : tt_final2, tt_T005.

************************************************
****Join Order 4 : INNER JOIN
************************************************
sort tt_final3 by D7_WAERS.
sort tt_TCURT by D8_WAERS.
CLEAR wa_final.
LOOP AT tt_final3
INTO wa_final.
CLEAR : wa_TCURT.
...
INTO wa_TCURT
from lv_tabix_frm.
IF wa_TCURT-D8_WAERS
 = wa_final-D7_WAERS.
MOVE wa_TCURT-D8_WAERS
 TO wa_final-D8_WAERS.
...
APPEND et_file_return.
ENDIF.
CLOSE CURSOR lv_FAGLFLEXT_dbcur.
DESCRIBE TABLE gt_result LINES lv_cnt.
WRITE : 'No of records in final table is :',lv_cnt.
************************************
